...
def chat_with_ai(data: ChatRequest):
    ...
    if not API_KEY or "ТУТ_ТВОЙ_РЕАЛЬНЫЙ_КЛЮЧ" in API_KEY:
        raise HTTPException(status_code=400, detail="Ты забыл вставить ключ в 10-ю строчку файла main.py!")

    system_prompt = f"""
Ты — AI-ассистент сервиса бронирования отеля.
ТЕКУЩИЙ СТАТУС БРОНИРОВАНИЯ:
- Паспорт получен: {"ДА" if data.passport_received else "НЕТ"}

ПРАВИЛА:
1. Если "Паспорт получен" = НЕТ: Объясни, что сначала необходимо предоставить паспорт, и дай ссылку: https://example.com/passport
2. Если "Паспорт получен" = ДА: Сообщи, что паспорт принят, и следующим этапом будет оплата залога.
Отвечай вежливо и коротко.
"""

    try:
        # Инициализируем клиента прямо здесь
        client = OpenAI(
            api_key=API_KEY,
        ...
        # Если VPN блочит, или ключ неверный, или лимиты — мы это увидим!
